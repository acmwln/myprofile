...
(function(e) {
	...
	e.extend(a.prototype, {
		...
		parseDate: function(t, i, s) {
			...
			var a, n, o, r, h = 0,
				...
				u = "string" != typeof l ? l : (this.nowDate).getFullYear() % 100 + parseInt(l, 10),
			...
			if (-1 === m ? m = (this.nowDate).getFullYear() : 100 > m && (m += (this.nowDate).getFullYear() - (this.nowDate).getFullYear() % 100 + (u >= m ? 0 : -100)), y > -1)
				for (g = 1, v = y;;) {
		...
		_getDefaultDate: function(e) {
			return this._restrictMinMax(e, this._determineDate(e, this._get(e, "defaultDate"), this.nowDate))
		},
		_determineDate: function(t, i, s) {
			var self = this;
			var a = function(e) {
					var t =self._getNow();
					// console.log('getNow()'+self._getNow());
					// cosole.log('nowDate'+self.nowDate);
				...
				n = function(i) {
					...
					for (var a = (i.toLowerCase().match(/^c/) ? e.datepicker._getDate(t) : null) || a.nowDate, n = a.getFullYear(), o = a.getMonth(), r = a.getDate(), h = /([+\-]?[0-9]+)\s*(d|D|w|W|m|M|y|Y)?/g, l = h.exec(i); l;) {
						switch (l[2] || "d") {
							case "d":
							case "D":
								r += parseInt(l[1], 10);
		...
		_setDate: function(e, t, i) {
			var s = !t,
				a = e.selectedMonth,
				n = e.selectedYear,
				o = this._restrictMinMax(e, this._determineDate(e, t,this.nowDate));
		...
		_generateHTML: function(e) {
			var t, i, s, a, n, o, r, h, l, u, d, c, p, f, m, g, v, y, b, _, x, w, k, T, D, S, N, M, C, A, P, I, z, H, F, E, W, L, O, j =this.nowDate,
		...
		_generateMonthYearHeader: function(e, t, i, s, a, n, o, r) {
			...
			if (y || (b += _ + (!n && g && v ? "" : "&#xa0;")), !e.yearshtml)
				if (e.yearshtml = "", n || !v) b += "<span class='ui-datepicker-year'>" + i + "</span>";
					...
					for (d = this._get(e, "yearRange").split(":"), c = (this.nowDate).getFullYear(), p = function(e) {
		...
		_isInRange: function(e, t) {
			...
			return h && (i = h.split(":"), s = (this.nowDate).getFullYear(), o = parseInt(i[0], 10), r = parseInt(i[1], 10), i[0].match(/[+\-].*/) && (o += s), i[1].match(/[+\-].*/) && (r += s)), (!a || t.getTime() >= a.getTime()) && (!n || t.getTime() <= n.getTime()) && (!o || t.getFullYear() >= o) && (!r || r >= t.getFullYear())
		},
		_getFormatConfig: function(e) {
			var t = this._get(e, "shortYearCutoff");
			return t = "string" != typeof t ? t : (this.nowDate).getFullYear() % 100 + parseInt(t, 10), {
				shortYearCutoff: t,
 ...
 var myCalendar = {  
    init:function(){
      ...
      //计算可预约日期始终为7天
      var nowDate = parseInt($date.getDate());
      var validDate = nowDate+self.minDate;
      var nowMonth = $date.getMonth()+1;
      self.nowMonth = $date.getMonth()+1;
      var startDate =self.calDate(validDate,nowMonth);
      var startDay = new Date(startDate).getDay();
      //计算maxDate
      self.maxDate = 10+self.minDate;//1
      if(startDay == 2 || startDay == 3){
        self.maxDate = 9+self.minDate;
      }else if(startDay == 4 || startDay == 5){
        self.maxDate = 12+self.minDate;
      }
      var endDate =self.calDate((nowDate+self.maxDate),nowMonth);
      self.delHoliday(startDate,endDate,nowDate,nowMonth);
    ...
    //计算起始日期 及终止日期
    calDate:function(validDate,nowMonth){
      var self =this;
      var start_month = nowMonth;
      var start_year =  self.getNow().getFullYear();
      var arr_mon1 = [1,3,5,7,8,10,12];
      var arr_mon2 = [4,6,9,11];
      if(validDate>28){
        if(nowMonth == 2){
          validDate-=28;
          start_month =(nowMonth+1)%12;
          // start_year  +=1;
        }else{
           for(var i=0;i<arr_mon1.length;i++){
             if(nowMonth == arr_mon1[i] && validDate>31){
              validDate -=31;
              start_month =nowMonth+1>12 ? (nowMonth+1)%12 :(nowMonth+1);
              if(nowMonth==12){
                start_year  +=1;
              }
              break;
           ...
           for(var i=0;i<arr_mon2.length;i++){
             if(nowMonth == arr_mon2[i] && validDate>30){
              validDate -=30;
             start_month =nowMonth+1>12 ? (nowMonth+1)%12 :(nowMonth+1);
              if(nowMonth==12){
                start_year  +=1;
      ...
      if(parseInt(start_month) < 10){
        start_month = '0'+parseInt(start_month);
      }
      if(parseInt(validDate) < 10){
        validDate = '0'+parseInt(validDate);
      }
      return start_year+'-'+start_month+'-'+validDate;

    },
     //扣除2015节假日 
    delHoliday:function(startDate,endDate,nowDate,nowMonth){
      var self = this;
      var tempMinDate = new Date(startDate).getDate();
      var tempMaxDate = new Date(endDate).getDate();
      //春节假期
      if(startDate>='2015-02-11' && endDate<"2015-02-26"){
       var date = parseInt(startDate.split('-')[2]);
       var dis = (26-date);
       // console.log("dis="+dis);
       self.maxDate+=dis;
       endDate =self.calDate((nowDate+self.maxDate),nowMonth);
       tempMaxDate = new Date(endDate).getDate();

      }else if(startDate<='2015-02-26' && endDate>="2015-02-26"){
        var date1 = parseInt(startDate.split('-')[2]);
        var date2 = parseInt(endDate.split('-')[2]);
        var mon = parseInt(endDate.split('-')[1]);
        if(mon==2){
        	var dis = (26-date1)+(date2-26);
        }else if(mon==3){
        	var dis = (26-date1);
        }       
        // console.log("dis="+dis);
      ...
      //清明假期
      if(startDate>='2015-03-24' && startDate<="2015-04-08"){
       self.maxDate+=1;
       endDate =self.calDate((nowDate+self.maxDate),nowMonth);
       tempMaxDate = new Date(endDate).getDate();
		}
      ...
      if(endDate>='2015-06-01' && endDate<="2015-06-15"){
        var date = parseInt(endDate.split('-')[2]);
        var dis = (15-date)+date;
        // console.log("dis="+dis);
      ...
      if(endDate>='2015-06-16' && endDate<="2015-06-26"){
        var date = parseInt(endDate.split('-')[2]);
        var dis = (26-date);
        // console.log("dis="+dis);
      ...
      //端午假期
      if(startDate>='2015-06-17' && startDate<="2015-06-24"){    
       self.maxDate+=1;
       var temp =nowDate+self.maxDate;
       endDate =self.calDate((nowDate+self.maxDate),nowMonth);
       
		tempMaxDate = new Date(endDate).getDate();
		 if(tempMaxDate == 27 || tempMaxDate == 28 || tempMaxDate == 29){tempMaxDate = 30;self.maxDate = 30;return;}
      }



       //国庆假期
      if(startDate>='2015-10-01' && startDate<="2015-10-07"){
       var dis = 7-parseInt(startDate.split('-')[2])+1;
       // console.log("dis="+dis);
       self.minDate+=dis;
       self.maxDate+=dis;
       endDate =self.calDate((nowDate+self.maxDate),nowMonth);
       tempMaxDate = new Date(endDate).getDate();
      }else if(endDate>='2015-10-01' && endDate<="2015-10-07"){
        var date = parseInt(endDate.split('-')[2]);
        var dis = (7-date)+(date-6);
        // console.log("dis="+dis);
      ...
      }else if(startDate<='2015-10-01' && endDate>="2015-10-07"){
        self.maxDate+=7;
        endDate =self.calDate((nowDate+self.maxDate),nowMonth);
        tempMaxDate = new Date(endDate).getDate();
      }  
      ...
      if(tempMaxDate < tempMinDate){
       self.numberOfMonths = [1,2];
      }

    },
    disableHoliday:function(){
      var holiday = {
        '0':[{"1":"元旦"},{"2":"休"},{"3":"休"}],
        '1':[{"11":"满"},{"12":"满"},{"13":"满"},{"14":"休"},{"15":"满"},{"16":"满"},{"17":"休"},{"18":"除夕"},{"19":"春节"},{"20":"休"},{"21":"休"},{"22":"休"},{"23":"休"},{"24":"休"},{"25":"满"}],
        '3':[{"4":"休"},{"5":"清明"},{"6":"休"},{"7":"休"}],
        '4':[{"1":"劳动节"},{"2":"休"},{"3":"休"}],
        '5':[
        {"1":"休"},{"2":"休"},{"3":"休"},{"4":"休"},{"5":"休"},{"6":"休"},{"7":"休"},{"8":"休"},{"9":"休"},{"10":"休"},
        {"11":"休"},{"12":"休"},{"13":"休"},{"14":"休"},{"15":"休"},
        {"20":"端午"},{"21":"休"},{"22":"休"},{"23":"休"}
        ],
        '8':[{"27":"中秋"}],
        '9':[{"1":"国庆"},{"2":"休"},{"3":"休"},{"4":"休"},{"5":"休"},{"6":"休"},{"7":"休"}]
      };
      for(var key in holiday){
        var arr = holiday[key];
        var $tdList = $('td[data-month='+key+']');
        // console.log($tdList);
        if($tdList.length>0){
          $tdList.each(function(){
            var $aEle = $(this).find('a');
            var dateVal = $.trim($aEle.text()).split('满')[0];
            // console.log(dateVal);
            for(var i=0;i<arr.length;i++){
              if(arr[i][dateVal]){
                $aEle.html(dateVal+'<span class="full">'+arr[i][dateVal]+'</span>').parent().addClass('ui-datepicker-unselectable ui-state-disabled');
              }
            }
          });
        }     
   ...
   show:function(){
		...
		$('#ui-datepicker-div').css('top',0).height(docHeight);
		$('.ui-datepicker-group').css('width','100%');
		$('.ui-datepicker-week-end').addClass('ui-datepicker-unselectable ui-state-disabled');
		$('tbody>tr>td:nth-child(7n+1)').addClass('ui-datepicker-unselectable ui-state-disabled');
		$('tbody>tr>td:nth-child(7n+1)').each(function(index,ele){
		   var temp_html = $(ele).find('a').html();
		  if(typeof(temp_html)!='undefined'){
		     $(ele).find('a').html(temp_html+'<span class="full">满</span>');

		  }
		});
		var $calender = $('.ui-datepicker-calendar');
		if($calender.length ==2 && self.nowMonth ==12){
		  $tr = $($($calender[1]).find('tr')[1]);
		 

		  $tr.find('a').each(function(index,ele){
		    if(index ==0 ||index==1){
		      var temp_html = $(ele).html();
		      $(ele).html(temp_html+'<span class="full">满</span>').parent().addClass('ui-datepicker-unselectable ui-state-disabled');
		    }

		  })
		}
		var $now=  self.getNow();
		$('.ui-datepicker-year').html($now.getFullYear()+'年');
		    var temp_mon = parseInt($($('.ui-datepicker-month')[1]).text().split('月')[0]);
		    var now_mon = parseInt($now.getMonth()+1);
		    if(temp_mon < now_mon){
		      $($('.ui-datepicker-year')[1]).html($now.getFullYear()+1+'年')
		}
		self.disableHoliday();
		$('.ui-datepicker-calendar').find('a').on('click',function(){
		    if($(this).parent().hasClass('ui-datepicker-unselectable')){
		       return false;
		     }
		     $('#dateTip').css('opacity','0');
		     // mq-update 2014-09-25
		     var today = $(this).text();
		     if(today.length==1){
		       today = '0'+today;
		     }

		     $(this).css({'background':'#f06b3e','border-radius':'5px','color':'#fff'}).html(today+'<br>预约');
		     var year = $(this).parents('table').siblings().find('.ui-datepicker-year').html().substr(0,4);

		     // mq-update 2014-09-25
		      var month =$(this).parents('table').prev().find('.ui-datepicker-month').html().split('月')[0];
		      if(month.length==2){
		        month = month;
		      }else{
		        month = '0'+month;
		     ...
		     if(today.indexOf('预约')>0){
		      today=today.split('<br>预约')[0];
		      //console.log(today)
		     }
		     var date = year+'-'+month+'-'+today;
		     $('#orderCalender').text(date).parent().removeClass('cgray');
